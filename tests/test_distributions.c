...
Parameters** create_1_parameters_dim(double* p, size_t dim){
	...
	for(int i = 0; i < dim; i++)
		Parameters_move(ps[0], new_Parameter("first", p[i], new_Constraint(0, INFINITY)));
	return ps;
}

Parameters** create_2_parameters_dim(const double* first, const double* second, size_t dim){
	Parameters** ps = malloc(sizeof(Parameters*)*2);
	ps[0] = new_Parameters_with_name("dim0", dim);
	ps[1] = new_Parameters_with_name("dim1", dim);
	for(size_t i = 0; i < dim; i++){
		Parameters_move(ps[0], new_Parameter("first", first[i], new_Constraint(0, INFINITY)));
		Parameters_move(ps[1], new_Parameter("second", second[i], new_Constraint(0, INFINITY)));
	}
	return ps;
}
...
double gsl_1p_logP(double (*lpdf)(const double, const double), const double* xs, size_t xdim, const double* p, size_t pdim){
	double logP = 0;
	if(pdim == 1){
...
double gsl_2p_logP(double (*lpdf)(const double, const double, const double), const double* xs, size_t xdim, const double* p, size_t pdim){
	double logP = 0;
	if(pdim == 1){
		for(size_t i = 0; i < xdim; i++){
...
double gsl_1p_dlogPdx(double (*lpdf)(const double, const double), const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps){
	double p0 = p[0];
	if(pdim > 1){
		p0 = p[idx];
	}
...
double gsl_1p_d2logPdx(double (*lpdf)(const double, const double), const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps) {
	double p0 = p[0];
	if(pdim > 1){
		p0 = p[idx];
	}
...
double gsl_2p_dlogPdx(double (*lpdf)(const double, const double, const double), const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps){
	double p0 = p[0];
	double p1 = p[1];
	if(pdim > 1){
		p0 = p[idx];
	...
	return (log(lpdf(xs[idx]+eps, p0, p1)) - log(lpdf(xs[idx]-eps, p0, p1)))/(2*eps);
}

double gsl_1p_dlogPdp(double (*lpdf)(const double, const double), const double* xs, size_t xdim, const double* p, size_t pdim, size_t idx, double eps){
	double dlogP = 0;
	if(pdim > 1)
		return (log(lpdf(xs[idx], p[idx]+eps)) - log(lpdf(xs[idx], p[idx]-eps)))/(2*eps);
	for(int i = 0; i < xdim; i++){
		dlogP += (log(lpdf(xs[i], p[0]+eps)) - log(lpdf(xs[idx], p[0]-eps)))/(2*eps);
	}
	return dlogP;
}

double gsl_2p_dlogPdp(double (*lpdf)(const double, const double, const double), const double* xs, size_t xdim, const double* p, size_t pdim, size_t dim, size_t idx, double eps){
	double dlogP = 0;
	double p0 = p[0];
	double p1 = p[1];
	if(dim == 0){
		if(pdim > 1)
			return (log(lpdf(xs[idx], p[idx]+eps, p[idx+pdim])) - log(lpdf(xs[idx], p[idx]-eps, p[idx+pdim])))/(2*eps);
			
		for(int i = 0; i < xdim; i++){
			dlogP += (log(lpdf(xs[i], p0+eps, p1)) - log(lpdf(xs[i], p0-eps, p1)))/(2*eps);
		}
	}
	else{	
		if(pdim > 1)
			return (log(lpdf(xs[idx], p[idx], p[idx+pdim]+eps)) - log(lpdf(xs[idx], p[idx], p[idx+pdim]-eps)))/(2*eps);

		for(int i = 0; i < xdim; i++){
			dlogP += (log(lpdf(xs[i], p0, p1+eps)) - log(lpdf(xs[i], p0, p1-eps)))/(2*eps);
		}
	}
	return dlogP;
}

double gsl_gamma_logP(const double* xs, size_t xdim, const double* p, size_t pdim, bool rate){
	if(rate){
...
double gsl_gamma_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps, bool rate){
	if(rate){
		double* p2 = clone_dvector(p, 2*pdim);
		for(int i = 0; i < pdim; i++) p2[i+pdim] = 1.0/p[i+pdim];
...
double gsl_gamma_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t dim, size_t idx, double eps){
...
double gsl_lognormal_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps){
	return gsl_2p_dlogPdx(&gsl_ran_lognormal_pdf, xs, xdim, idx, p, pdim, eps);
}

double gsl_lognormal_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t dim, size_t idx, double eps){
...
double gsl_beta_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps){
	return gsl_2p_dlogPdx(&gsl_ran_beta_pdf, xs, xdim, idx, p, pdim, eps);
}

double gsl_beta_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t dim, size_t idx, double eps){
	return gsl_2p_dlogPdp(gsl_ran_beta_pdf, xs, xdim, p, pdim, dim, idx, eps);
}


double gsl_kumaraswamy_logP(const double* xs, size_t xdim, const double* p, size_t pdim){
	double logP = 0;
	if(pdim == 1){
		for(size_t i = 0; i < xdim; i++){
			logP += log(p[1]*p[0]*pow(xs[i], p[0] - 1.0) * pow( 1.0 - pow(xs[i], p[0]), p[1] - 1.0));
		}
	}
	else{
		for(size_t i = 0; i < xdim; i++){
			logP += log(p[i+pdim]*p[i]*pow(xs[i], p[i] - 1.0) * pow( 1.0 - pow(xs[i], p[i]), p[i+pdim] - 1.0));
		}	
	}
...
double gsl_kumaraswamy_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps){
	gsl_function F;
...
double gsl_kumaraswamy_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t idx, double eps){
	return 0;
}
...
//MARK: Normal
double gsl_normal_logP(const double* xs, size_t xdim, const double* p, size_t pdim, bool sigma){
...
double gsl_normal_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps, bool sigma){
	double* newxs = center_xs(xs, xdim, p, pdim);
	double* newp = clone_dvector(p+pdim, pdim);
	double dlogPdx;
	if(!sigma){
		for(int i = 0; i < pdim; i++) newp[i] = sqrt(1.0/p[pdim+i]);
		dlogPdx = gsl_1p_dlogPdx(&gsl_ran_gaussian_pdf, newxs, xdim, idx, newp, pdim, eps);
	}
	else{
		dlogPdx = gsl_1p_dlogPdx(&gsl_ran_gaussian_pdf, newxs, xdim, idx, newp, pdim, eps);
	}
...
double gsl_normal_d2logPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps, bool sigma){
	double* newxs = center_xs(xs, xdim, p, pdim);
	double* newp = clone_dvector(p+pdim, pdim);
	double dlogPdx;
	if(!sigma){
		for(int i = 0; i < pdim; i++) newp[i] = sqrt(1.0/p[pdim+i]);
		dlogPdx = gsl_1p_d2logPdx(&gsl_ran_gaussian_pdf, newxs, xdim, idx, newp, pdim, eps);
	}
	else{
		dlogPdx = gsl_1p_d2logPdx(&gsl_ran_gaussian_pdf, newxs, xdim, idx, newp, pdim, eps);
	}
...
double gsl_normal_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t idx, double eps, bool sigma){
	double* newxs = center_xs(xs, xdim, p, pdim);
	double* newp = clone_dvector(p+pdim, pdim);
	double result;
	if(!sigma){
...
//MARK: HalfNormal
double gsl_halfnormal_logP(const double* xs, size_t xdim, const double* p, size_t pdim, bool sigma){
	return gsl_normal_logP(xs, xdim, p, pdim, sigma)/2.0;
}

double gsl_halfnormal_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps, bool sigma){
	return gsl_normal_dlogPdx(xs, xdim, idx, p, pdim, eps, sigma);
}

double gsl_halfnormal_d2logPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps, bool sigma){
	return gsl_normal_d2logPdx(xs, xdim, idx, p, pdim, eps, sigma);
}

double gsl_halfnormal_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t idx, double eps, bool sigma){
...
double gsl_cauchy_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps){
	double* newxs = center_xs(xs, xdim, p, pdim);
	double dlogPdx = gsl_1p_dlogPdx(&gsl_ran_cauchy_pdf, newxs, xdim, idx, p+pdim, pdim, eps);
	free(newxs);
	return dlogPdx;
}

double gsl_cauchy_d2logPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps){
	double* newxs = center_xs(xs, xdim, p, pdim);
	double dlogPdx = gsl_1p_d2logPdx(&gsl_ran_cauchy_pdf, newxs, xdim, idx, p+pdim, pdim, eps);
	free(newxs);
	return dlogPdx;
}

double gsl_cauchy_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t idx, double eps){
	double* newxs = center_xs(xs, xdim, p, pdim);
	double dlogPd = gsl_1p_dlogPdp(&gsl_ran_cauchy_pdf, newxs, xdim, p+pdim, pdim, idx, eps);
	free(newxs);
	return dlogPd;
}

double gsl_exponential_logP(const double* xs, size_t xdim, const double* p, size_t pdim, bool ismean){
...
double gsl_exponential_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps, bool ismean){
	double* newp = clone_dvector(p, pdim);
	if(!ismean){
		for(int i = 0; i < pdim; i++) newp[i] = 1.0/p[i];
	}
	double res = gsl_1p_dlogPdx(&gsl_ran_exponential_pdf, xs, xdim, idx, newp, pdim, eps);
	free(newp);
	return res;
}

double gsl_exponential_d2logPdx(const double* xs, size_t xdim, size_t idx, const double* p, size_t pdim, double eps, bool ismean){
	double* newp = clone_dvector(p, pdim);
	if(!ismean){
		for(int i = 0; i < pdim; i++) newp[i] = 1.0/p[i];
	}
	double res = gsl_1p_d2logPdx(&gsl_ran_exponential_pdf, xs, xdim, idx, newp, pdim, eps);
	free(newp);
	return res;
}

double gsl_exponential_dlogPdp(const double* xs, size_t xdim, const double* p, size_t pdim, size_t idx, double eps, bool ismean){
	double* newp = clone_dvector(p, pdim);
	if(!ismean){
		for(int i = 0; i < pdim; i++) newp[i] = 1.0/p[i];
	}
...
double gsl_dirichlet_dlogPdx(const double* xs, size_t xdim, size_t idx, const double* p, double eps){
...
char* test_gamma_distribution_dim(Parameters** ps, double* p, size_t pdim, bool israte){
	size_t xdim = 2;
	double x[2] = {2, 4};
	Parameters* xs = create_x_dim(x, xdim);
	DistributionModel* dm = NULL;
	if(israte){
		dm = new_GammaDistributionModel_with_parameters(ps, xs, DISTRIBUTION_GAMMA_SHAPE_RATE);
	}
	else{
		dm = new_GammaDistributionModel_with_parameters(ps, xs, DISTRIBUTION_GAMMA_SHAPE_SCALE);
	}
...
char* test_exponential_distribution_dim(Parameters** ps, double* p, size_t pdim, bool ismean){
	size_t xdim = 2;
	double x[2] = {2, 4};
	Parameters* xs = create_x_dim(x, xdim);
	...
	DistributionModel* dm = NULL;
	if(ismean){
		dm = new_ExponentialDistributionModel_with_parameters(ps, xs, DISTRIBUTION_EXPONENTIAL_MEAN);
	}
	else{
		dm = new_ExponentialDistributionModel_with_parameters(ps, xs, DISTRIBUTION_EXPONENTIAL_RATE);
	}
	Model* model = new_DistributionModel2("dist", dm);
...
char* test_normal_distribution_issigma(bool issigma){
	...
	DistributionModel* dm = NULL;
	if(issigma){
		dm = new_NormalDistributionModel_with_parameters(ps, xs, DISTRIBUTION_NORMAL_MEAN_SIGMA);
	}
	else{
		dm = new_NormalDistributionModel_with_parameters(ps, xs, DISTRIBUTION_NORMAL_MEAN_TAU);
	}
	Model* model = new_DistributionModel2("dist", dm);
...
char* test_normal_distribution_tau(){
	return test_normal_distribution_issigma(false);
}
...
char* test_dirichlet_distribution(){
	...
	logP = model->logP(model);
	logP2 = gsl_ran_dirichlet_lnpdf(xdim, p, x);
	mu_assert(fabs(logP - logP2) < .000001, "logP after changing dirichlet not matching");
