...
char* test_skyride(){
	...
	for(int i = 0; i < 3; i++){
		...
		mu_assert(fabs(dlogPdp - true_gradient[i]) < 0.00001, "d.logP/d.theta not matching");
	}
	for(int i = 3; i < 6; i++){
		double dlogPdp = model->dlogP(model, Parameters_at(ps, i));
		mu_assert(fabs(dlogPdp - true_gradient[i]) < 0.00001, "d.logP/d.t not matching");
	}
...
// 	for(int i = 0; i < 3; i++){
// 		data.index = i+3;
// 		double dlogPdp2 = gsl_coalescent_dlogPdx(&data, Parameters_value(reparams, i), 0.0001);
// 	}
	model->free(model);
	mtree->free(mtree);
	free_Parameters(ps);
...
char* test_constant_data(){
	...
	Parameter* N = new_Parameter("", value, new_Constraint(0, INFINITY));
	Coalescent* coal = new_ConstantCoalescent_with_data(N, times, coalescent, intervalCount);
