...
Model* new_BetaDistributionModel_from_json(json_node* node, Hashtable* hash){
    ...
    if (file != NULL) {
        ...
        for (int i = 0; i < paramCount; i++) {
            ...
            Parameters_move(parameters[0], new_Parameter("alpha", alpha, new_Constraint(0, 1)));
            Parameters_move(parameters[1], new_Parameter("beta", beta, new_Constraint(0, 1)));
        ...
        for (int i = 0; i < Parameters_count(x); i++) {
            Parameters_move(parameters[0], new_Parameter("alpha", 0, new_Constraint(0, 1)));
            Parameters_move(parameters[1], new_Parameter("beta", 1, new_Constraint(0, 1)));
        }
    }
    else{
